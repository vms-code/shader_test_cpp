...
PerspectiveCamera::PerspectiveCamera(float fov, float aspect, float near, float far) : Camera(near, far), fov(fov), aspect(aspect) {
...
void PerspectiveCamera::updateProjectionMatrix() {
    ...
    if (this->view && this->view->enabled) {
        const auto fullWidth = static_cast<float>(view->fullWidth), fullHeight = static_cast<float>(view->fullHeight);
